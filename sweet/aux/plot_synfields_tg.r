...
obsnet_or <- obsnet
print("-- obsNet --")
print( cbind(obsnet$x,obsnet$y,obsnet$z,obsnet$isin))
print(obsnet$n)
#
#------------------------------------------------------------------------------
...
#print(res[[1]][iso])
  ...
  axis(2,cex.axis=2)
  mtext(side=2,text="Elevation [m a.m.s.l.]",line=3,cex=2)
...
q()
  ...
  plot(obsnet$x,obsnet$y,col="white", xlab="",ylab="",cex.axis=2)
  col<-gray(seq(0.2,0.8,length=15))
  image(r,add=T,col=col,breaks=seq(150,2000,length=16))
  points(obsnet$x,obsnet$y,pch=21,bg="white",cex=3)
  points(obsnet$x[obsnet$isin==1],obsnet$y[obsnet$isin==1],pch=22,bg="white",cex=4)
  par(new=T)
  par(mar=c(40,40,0.5,0.5))
  ex<-as(extent(range(obsnet$x)[1],range(obsnet$x)[2],range(obsnet$y)[1],range(obsnet$y)[2]),'SpatialPolygons'); crs(ex)<-CRS("+proj=lcc +lat_0=63 +lon_0=15 +lat_1=63 +lat_2=63 +no_defs +R=6.371e+06")
  ex1<-as(extent(-550000,-100000,-500000,0),'SpatialPolygons'); crs(ex1)<-CRS("+proj=lcc +lat_0=63 +lon_0=15 +lat_1=63 +lat_2=63 +no_defs +R=6.371e+06")
  r<-crop(raux$stack,ex1)
  ...
  image(r,col=c("cyan",col),breaks=c(0,seq(1,2200,length=16)),axes=F,xlab="",ylab="")
  plot(b,add=T,axes=F,xlab="",ylab="")
  plot(ex,add=T,lwd=4,col="gold",axes=F,xlab="",ylab="")
  box()
...
#  text(obsnet$x[c],obsnet$y[c],round(res[[2]][c],1),cex=1.5,col="darkred")
  dev.off()
print(length(obsnet$y))
print(length(which(obsnet$isin==1)))
q()
