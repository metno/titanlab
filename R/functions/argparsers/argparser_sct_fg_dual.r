  ...
  p <- add_argument(p, "--sct_fg_dual",
                    help="do SCT dual",
                    flag=T)

  p <- add_argument(p, "--prio.sct_fg_dual",
  ...
  p <- add_argument(p, "--i.sct_fg_dual",
                    help="number of SCT dual iterations. In case of more tests (N>1), then the whole sequence is repeated several times.",
                    type="integer",
                    default=1)

  p <- add_argument(p, "--inner_radius.sct_fg_dual",
                    help="radius (m) of the inner circle. Can be a N-vector when a sequence of N tests is needed.",
                    type="numeric",
                    default=NA,
                    nargs=Inf)

  p <- add_argument(p, "--outer_radius.sct_fg_dual",
                    help="radius (m) of the outer circle. Either one value valid for all N or N values.",
                    type="numeric",
                    default=NA,
                    nargs=Inf)

  p <- add_argument(p, "--num_min_outer.sct_fg_dual",
  ...
  p <- add_argument(p, "--max_horizontal_scale.sct_fg_dual",
  ...
  p <- add_argument(p, "--kth_closest_obs_horizontal_scale.sct_fg_dual",
  ...
  p <- add_argument(p, "--vertical_scale.sct_fg_dual",
  ...
  p <- add_argument(p, "--thr_relinfo.sct_fg_dual",
  ...
  p <- add_argument(p, "--event_thresholds.sct_fg_dual",
  ...
  p <- add_argument(p, "--conditions.sct_fg_dual",
  ...
  p <- add_argument(p, "--fg_event_thresholds.sct_fg_dual",
                    help="event threshold used to divide the first-guess data into \"yes, the event happend\" or \"no, the event did not happen\". Either one value valid for all N or N values.",
                    type="numeric",
                    default=NA,
                    nargs=Inf)

  p <- add_argument(p, "--break.sct_fg_dual",
