...
for (i in length(col):1) {
  ...
  if ( length( ix<- which( dat$value>br[i] & dat$value<=br[i+1] & dat$ge == 1)) > 0) 
    points( dat$lon[ix], dat$lat[ix], pch=21, bg=col[i],col="red", cex=2)
  if ( length( ix<- which( dat$value>br[i] & dat$value<=br[i+1] & dat$dqc == 4)) > 0) 
